...
│   {
│       subtype: "fill_blank" | "multiple_choice",
│       content: "题目",
│       answer: "答案",
│       options: "选项或填空词"
│   }
...
**对话1** (A): 他学理科，他数学和化学的成绩不错，考试都在九十分以上，他不喜欢上文化课。
- 註釋：对话一
...
**字段说明**：
- `**对话X**:`：对话编号（必填，X 为数字）
- `(A)` 或 `(B)`：說話者字母標記（必填，A、B、C...）
- `(A: 名稱)`：可選的說話者名稱（如果提供，會顯示 "A: 名稱"，否則只顯示 "A"）
- `- 註釋：对话一`：可選的對話組標記（用於區分不同的對話場景，如「对话一」「对话二」）
- 每行一个完整句子，不需要用 `|` 分隔
- 对话内容直接作为完整句子存储

**注意**：
- 对话属于 `phrases` 数组的一部分，`subtype: "dialogue"`
- 每个对话项目显示为一个独立的对话框
- 多个对话项目按顺序显示，形成完整对话
- **對話分組**：
  - 如果多個對話有相同的 `- 註釋：` 值，它們會被歸為同一組
  - 當 `註釋` 值改變時，會顯示為組標題（如「对话一」「对话二」）
  - 組標題會顯示在該組第一個對話的上方
- **視覺樣式**：
  - 說話者 A：左側，藍色氣泡
  - 說話者 B：右側，綠色氣泡
...
#### 3.1 帶例句的寫法（Grammar with Examples）

若要在 lesson.html 的語法區塊顯示「💡 例句」，須在**同一行**的英文說明後加上 ` Examples: `，並用 `|` 分隔多個例句；**例句須以中文句號、問號或驚嘆號結尾**（`。` `？` `！`），解析器才會識別為 `subtype: 'example'`。

**格式**（同一行，英文後接 Examples）：
```markdown
## 识字16 - 语法

**规则1** (以主谓结构作谓语的句子…) | A sentence in which a subject-predicate construction serves as its predicate, e.g. Structure: S S adj. Examples: 我肚子不舒服。| 他头疼。| 她眼睛美丽。

**规则2** ("别"的用法举例) | Usage of 别. Structure: S 别 V. Examples: 你别去上课了。| 你别让孩子去上学了。
```

**要點**：
- 英文說明與例句之間寫：` Examples: `（注意空格）
- 例句用 `|` 分隔，例如：`例句1。| 例句2。| 例句3。`
- 每條例句必須以 `。`、`？` 或 `！` 結尾，否則不會被當成例句
...
- `- 答案位置：X` 或 `- 答案：X`：答案位置或答案（必填）
  - **多选时**：用英文竖线 `|` 分隔，例如 `- 答案位置：C|D`、`- 答案：B|C|D`（lesson.html 会按多选校验）
- `- 填空词：词语`：填空题的填空词（填空题必填）
- `- 选项：A.选|B.选|C.选|D.选`：选择题的选项（选择题必填，用 `|` 分隔）

**注意**：
- 对话不属于练习部分，应使用 `## 识字X - 对话` 格式（见 2.1 节）
...
- **对话**：`**对话X** (A): 內容` 或 `**对话X** (A: 名稱): 內容` → `{content: "內容", subtype: "dialogue", type: "phrases", speaker: "A", speaker_name: "名稱（可選）"}`
...
- [ ] 表格格式正确（包含表头分隔线 `|----|----|----|`）
- [ ] 所有必填字段都有值（拼音、英文等）
- [ ] 对话格式正确（`## 识字X - 对话` + `**对话X** (A): 完整句子` 或 `**对话X** (A: 名稱): 完整句子`）
- [ ] 练习格式正确（新格式或旧格式都可以）
- [ ] 语法规则格式正确（`**规则X** (中文) | 英文`）
- [ ] 没有多余的空白行或格式错误
